...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("Test (2A): disconnect %d \n", leader1)
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	DPrintf("Test (2A): disconnect and rejoin leader %d \n", leader1)
	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	DPrintf("Test (2A): disconnect 1-2 leader %d \n", leader1)
	DPrintf("Test (2A): disconnect 2-2 leader %d \n", (leader2+1)%servers)
	...
	DPrintf("Test (2A): rejoin a node %d \n", leader1)
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	DPrintf("Test (2A): rejoin last node %d \n", leader1)
